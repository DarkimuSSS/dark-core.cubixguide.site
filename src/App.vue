...
const MarketplacePage = defineAsyncComponent(() => import('./components/MarketplacePage.vue'));
...
const isBecomeAuthorOpen = ref(false);
...
// Theme Switcher State: 'dark' | 'light' | 'emerald' | 'crimson'
type ThemeMode = 'dark' | 'light' | 'emerald' | 'crimson';
...
const applyTheme = (theme: ThemeMode) => {
  ...
  html.classList.remove('theme-dark', 'theme-light', 'theme-emerald', 'theme-crimson');
  html.classList.add(`theme-${theme}`);
};

const toggleTheme = () => {
  const themes: ThemeMode[] = ['dark', 'light', 'emerald', 'crimson'];
...
const syncFromUrlPath = () => {
  ...
  } else if (tab === 'market' || tab === 'Маркетплейс' || tab === 'Маркет') {
    mode.value = 'market';
  } else if (tab === 'assets' || tab === 'Ассеты') {
    mode.value = 'assets';
  ...
  <div class="h-screen w-screen overflow-hidden bg-[#0c0d0e] text-[#e2e8f0] font-sans antialiased flex flex-col">
    ...
    <header class="bg-[#121417]/95 backdrop-blur-xl border-b border-[#262a30] h-16 shrink-0 px-4 sm:px-6 flex items-center justify-between z-40 shadow-2xl transition-all select-none gap-4">
      ...
      <div class="flex items-center gap-2.5 shrink-0">
        ...
        <button
          type="button"
          @click="isBecomeAuthorOpen = true"
          class="px-2.5 sm:px-3 py-2 rounded-xl text-xs font-extrabold flex items-center gap-1.5 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 transition-all duration-300 shadow-md cursor-pointer shrink-0"
          title="Как стать Автором DarkCore Wiki"
        >
        ...
        <div class="max-w-[1600px] mx-auto w-full flex-1 flex flex-col justify-between">
          ...
          <div v-else-if="mode === 'author_dashboard' && isAuthenticated" class="px-3 sm:px-6 pt-4">
            <AuthorDashboardModal
              ...
              @open-market="mode = 'market'"
            />
          </div>

          <!-- STANDALONE MARKETPLACE PAGE VIEW (tab=market) -->
          <div v-else-if="mode === 'market'">
            <MarketplacePage
              :current-username="currentUsername || ''"
              @go-home="mode = 'home'"
            />
          ...
          <!-- ASSETS HUB FULL PAGE VIEW (tab=assets) -->
    ...
    <!-- Become Author Modal -->
    <BecomeAuthorModal
      :is-open="isBecomeAuthorOpen"
      @close="isBecomeAuthorOpen = false"
      @open-invite-input="isAuthModalOpen = true"
    />
  </div>
</template>
