...
<template>
  <div class="min-h-screen bg-[#0c0d0e] text-[#e2e8f0] pb-16">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 space-y-6">
      ...
      <!-- 3. ANIMATED TIER STRUCTURE VISUALIZER (3D Isometric Layer Visualizer) -->
      <div class="p-6 sm:p-8 rounded-3xl bg-[#121417] border border-rose-500/20 shadow-2xl space-y-6 relative overflow-hidden">
        <div class="absolute -bottom-16 -left-16 w-48 h-48 bg-rose-600/10 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div class="flex items-center justify-between flex-wrap gap-4 pb-4 border-b border-white/10">
          <div>
            <h3 class="text-lg font-black text-white flex items-center gap-2.5">
              <IconRenderer name="Layers" size="20" class="text-rose-400" />
              <span>Послойная схема постройки Алтаря (Tier {{ selectedTier }})</span>
            </h3>
            <p class="text-xs text-slate-400">Наглядный чертеж каждого слоя для постройки в Minecraft</p>
          </div>

          <div class="flex items-center gap-2 font-mono text-xs font-extrabold text-rose-400 bg-rose-500/10 px-3 py-1.5 rounded-xl border border-rose-500/20">
            <span class="w-2 h-2 rounded-full bg-rose-400 animate-ping"></span>
            <span>Размер: {{ currentTierInfo.dimensions }}</span>
          </div>
        </div>

        <!-- Tier Layers Grid Representation -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
          
          <!-- Layer 1: Altar Core / Center -->
          <div class="p-5 rounded-2xl bg-[#090a0c] border border-white/5 space-y-3 relative group hover:border-rose-500/30 transition-all">
            <div class="flex items-center justify-between text-xs font-bold">
              <span class="text-rose-400 flex items-center gap-1.5">
                <span class="w-2 h-2 rounded-full bg-rose-500"></span>
                Слой 1 — Центральное кольцо
              </span>
              <span class="text-slate-500">Y = 0</span>
            </div>
            
            <!-- Isometric Layer Blueprint Diagram -->
            <div class="h-44 rounded-xl bg-[#0e1012] border border-white/10 flex items-center justify-center relative overflow-hidden group-hover:shadow-[0_0_20px_rgba(244,63,94,0.1)] transition-all">
              <div class="grid grid-cols-3 gap-1 p-3">
                <div class="w-8 h-8 rounded-lg bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[10px] text-rose-300 font-mono font-bold">R</div>
                <div class="w-8 h-8 rounded-lg bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[10px] text-rose-300 font-mono font-bold">R</div>
                <div class="w-8 h-8 rounded-lg bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[10px] text-rose-300 font-mono font-bold">R</div>
                
                <div class="w-8 h-8 rounded-lg bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[10px] text-rose-300 font-mono font-bold">R</div>
                <!-- ALTAR CENTER BLOCK WITH ANIMATED PULSE -->
                <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-600 to-red-600 border border-white/40 shadow-lg shadow-rose-950/80 flex items-center justify-center text-white text-xs font-black animate-pulse">
                  А
                </div>
                <div class="w-8 h-8 rounded-lg bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[10px] text-rose-300 font-mono font-bold">R</div>
            ...
            <p class="text-[11px] text-slate-400">8 Рун (R) по периметру вокруг центрального блока Алтаря (А).</p>
          </div>

          <!-- Layer 2: Corner Pillars & Air Clearance -->
          <div class="p-5 rounded-2xl bg-[#090a0c] border border-white/5 space-y-3 relative group hover:border-amber-500/30 transition-all">
            <div class="flex items-center justify-between text-xs font-bold">
              <span class="text-amber-400 flex items-center gap-1.5">
                <span class="w-2 h-2 rounded-full bg-amber-500"></span>
                Слой 2 — Столбы подпорки
              </span>
              <span class="text-slate-500">Y = -1</span>
            </div>
            
            <div class="h-44 rounded-xl bg-[#0e1012] border border-white/10 flex items-center justify-center relative overflow-hidden group-hover:shadow-[0_0_20px_rgba(245,158,11,0.1)] transition-all">
              <div class="grid grid-cols-5 gap-1 p-2">
                <div class="w-6 h-6 rounded bg-amber-500/30 border border-amber-400 text-amber-200 text-[9px] font-bold flex items-center justify-center">П</div>
                <div class="w-6 h-6 rounded bg-white/5 border border-white/5"></div>
                <div class="w-6 h-6 rounded bg-white/5 border border-white/5"></div>
                <div class="w-6 h-6 rounded bg-white/5 border border-white/5"></div>
                <div class="w-6 h-6 rounded bg-amber-500/30 border border-amber-400 text-amber-200 text-[9px] font-bold flex items-center justify-center">П</div>
                
                <div class="w-6 h-6 rounded bg-white/5 border border-white/5"></div>
                <div class="w-6 h-6 rounded bg-white/10 border border-white/15"></div>
                <div class="w-6 h-6 rounded bg-white/10 border border-white/15"></div>
                <div class="w-6 h-6 rounded bg-white/10 border border-white/15"></div>
                <div class="w-6 h-6 rounded bg-white/5 border border-white/5"></div>
                ...
                <div class="w-6 h-6 rounded bg-white/5 border border-white/5"></div>
                <div class="w-6 h-6 rounded bg-white/10 border border-white/15"></div>
                <div class="w-6 h-6 rounded bg-rose-500/30 border border-rose-500/40 text-rose-300 text-[9px] font-bold flex items-center justify-center">R</div>
                <div class="w-6 h-6 rounded bg-white/10 border border-white/15"></div>
                <div class="w-6 h-6 rounded bg-white/5 border border-white/5"></div>
            ...
            <p class="text-[11px] text-slate-400">4 угловых пилона (П) и дополнительное кольцо ступеней рун.</p>
          </div>

          <!-- Layer 3: Pylons & Caps (Beacons / Crystals) -->
          <div class="p-5 rounded-2xl bg-[#090a0c] border border-white/5 space-y-3 relative group hover:border-cyan-500/30 transition-all">
            <div class="flex items-center justify-between text-xs font-bold">
              <span class="text-cyan-400 flex items-center gap-1.5">
                <span class="w-2 h-2 rounded-full bg-cyan-400"></span>
                Слой 3 — Вершины пилонов
              </span>
              <span class="text-slate-500">Y = +2</span>
            </div>
            
            <div class="h-44 rounded-xl bg-[#0e1012] border border-white/10 flex items-center justify-center relative overflow-hidden group-hover:shadow-[0_0_20px_rgba(6,182,212,0.1)] transition-all">
              <div class="grid grid-cols-5 gap-1 p-2">
                <div class="w-6 h-6 rounded bg-cyan-500/40 border border-cyan-300 text-white text-[9px] font-black flex items-center justify-center shadow-md animate-pulse">К</div>
            ...
            <p class="text-[11px] text-slate-400">4 вершины угловых столбов (К) — Светящийся камень, Маяки или Кристаллы.</p>
          </div>
