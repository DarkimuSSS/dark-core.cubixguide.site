...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fadeIn">
    ...
    <div class="relative w-full max-w-2xl">
      ...
      <!-- EXACT ORIGINAL COMPACT MODAL CONTAINER -->
      <div class="bg-[#16181a] border border-[#26292d] w-full rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 max-h-[85vh] overflow-y-auto custom-scrollbar relative">
        ...
        <template v-else>
          ...
          <!-- AUTHOR CUSTOM COVER BANNER (NEATLY INTEGRATED INSIDE THE CARD) -->
          <div v-if="profile.bannerUrl" class="h-28 sm:h-36 w-full rounded-2xl overflow-hidden border border-[#26292d] relative shadow-lg">
            <img :src="profile.bannerUrl" class="w-full h-full object-cover" />
            <div class="absolute inset-0 bg-gradient-to-t from-[#16181a]/80 via-transparent to-transparent"></div>
          </div>
