  ...
  <div v-if="isFullPage" class="max-w-6xl mx-auto space-y-6 pb-12">
    ...
    <div v-else-if="analyticsData" class="space-y-6">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        <!-- Total Views -->
        <div class="p-4 sm:p-5 rounded-2xl bg-[#141619] border border-cyan-500/30 shadow-md space-y-1">
          <div class="text-[10px] sm:text-xs font-extrabold uppercase tracking-wider text-cyan-400 flex items-center gap-1.5">
            <IconRenderer name="Eye" size="14" />
            Всего Просмотров
          </div>
          <div class="text-2xl sm:text-3xl font-black text-white font-mono">{{ analyticsData.totalViews }}</div>
          <div class="text-[10px] text-dark-muted">по всем вашим гайдам</div>
        </div>

        <!-- Total Guides -->
        <div class="p-4 sm:p-5 rounded-2xl bg-[#141619] border border-[#2b2f36] shadow-md space-y-1">
          <div class="text-[10px] sm:text-xs font-extrabold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
            <IconRenderer name="FileText" size="14" />
            Всего Статей
          </div>
          <div class="text-2xl sm:text-3xl font-black text-white font-mono">{{ analyticsData.totalGuides }}</div>
          <div class="text-[10px] text-dark-muted">создано автором</div>
        </div>

        <!-- Published Count -->
        <div class="p-4 sm:p-5 rounded-2xl bg-[#141619] border border-emerald-500/30 shadow-md space-y-1">
          <div class="text-[10px] sm:text-xs font-extrabold uppercase tracking-wider text-emerald-400 flex items-center gap-1.5">
            <IconRenderer name="CheckCircle2" size="14" />
            Опубликовано
          </div>
          <div class="text-2xl sm:text-3xl font-black text-emerald-400 font-mono">{{ analyticsData.publishedCount }}</div>
          <div class="text-[10px] text-dark-muted">активны в каталоге</div>
        </div>

        <!-- Pending & Drafts Count -->
        <div class="p-4 sm:p-5 rounded-2xl bg-[#141619] border border-amber-500/30 shadow-md space-y-1">
          <div class="text-[10px] sm:text-xs font-extrabold uppercase tracking-wider text-amber-400 flex items-center gap-1.5">
            <IconRenderer name="Clock" size="14" />
            Черновики / В работе
          </div>
          <div class="text-2xl sm:text-3xl font-black text-amber-400 font-mono">{{ analyticsData.draftCount + analyticsData.pendingCount }}</div>
      ...
      <div class="space-y-3">
        <h4 class="text-xs font-extrabold uppercase tracking-wider text-slate-200 flex items-center gap-2">
          <IconRenderer name="BarChart3" size="16" class="text-cyan-400" />
          <span>Рейтинг просмотров ваших гайдов</span>
        </h4>

        <div class="bg-[#141619] border border-[#2b2f36] rounded-2xl overflow-hidden shadow-lg">
          <div v-if="analyticsData.guides.length === 0" class="p-8 text-center text-xs text-dark-muted">
          ...
          <div v-else class="divide-y divide-[#26292d]">
            <div 
              ...
              :key="guide.id"
              class="p-4 hover:bg-[#1c1f24] transition-colors flex items-center justify-between gap-3 group"
            >
              <div class="min-w-0 flex-1 space-y-1">
                ...
                <h5 class="text-sm font-extrabold text-white group-hover:text-cyan-300 transition-colors truncate">
                  {{ guide.title }}
                </h5>
              </div>
              ...
              <div class="flex items-center gap-4 shrink-0">
                <div class="text-right">
                  <div class="text-sm font-black text-cyan-400 font-mono flex items-center justify-end gap-1">
                    <IconRenderer name="Eye" size="14" />
                ...
                <button 
                  ...
                  class="p-2 rounded-xl bg-[#22262c] hover:bg-cyan-600 text-slate-300 hover:text-white transition-all cursor-pointer shadow-sm flex items-center gap-1.5 text-xs font-bold px-3"
                  title="Открыть гайд"
                >
                  <span>Открыть</span>
      ...
      <div v-if="analyticsData.recentLogs.length > 0" class="space-y-3">
        <h4 class="text-xs font-extrabold uppercase tracking-wider text-slate-200 flex items-center gap-2">
          <IconRenderer name="Activity" size="16" class="text-emerald-400" />
          <span>Лента недавней активности читателей</span>
        </h4>

        <div class="bg-[#141619] border border-[#2b2f36] rounded-2xl p-4 space-y-2 shadow-lg">
          <div 
            v-for="log in analyticsData.recentLogs.slice(0, 10)" 
            :key="log.id"
            class="flex items-center justify-between text-xs py-2 border-b border-[#26292d]/50 last:border-none"
          >
